...
export function BracketCard({ match, x, y, width = 410, height = 166, label, color = '#ffda74', schedule = true, size = 19 }) {
  ...
  return <g>
    <text x={x} y={y - 13} fill={color} fontSize={size} className='poster-title' fontWeight='600' letterSpacing='1'>{label}</text>
...
export default function InstagramBracket({ tournament, matches, cup }) {
  ...
  return <svg className='playoff-type' xmlns='http://www.w3.org/2000/svg' viewBox='0 0 1080 1350' role='img' aria-label={`Playoffs copa ${cup} de ${tournament.name}`} style={{ width: '100%', display: 'block', fontFamily: 'PosterInter, sans-serif' }}>
    ...
    <text x='630' y='191' fill='white' fontSize='70' className='poster-title' fontWeight='600' textAnchor='middle' letterSpacing='-2'>{tournament.category} · PLAYOFFS</text>
