...
function TournamentLogos({ y, left, right, size }) {
  return <g aria-label='Logos de Copa Kenia y Copa Comercial Eldorado'>
    <rect x={left} y={y} width={size} height={size} rx='18' fill='white' fillOpacity='.96' />
    <image href='/images/copa-kenia-logo.png' x={left + 7} y={y + 7} width={size - 14} height={size - 14} preserveAspectRatio='xMidYMid meet' />
    <rect x={right} y={y} width={size} height={size} rx='18' fill='white' fillOpacity='.96' />
    <image href='/images/copa-comercial-eldorado-logo.png' x={right + 7} y={y + 7} width={size - 14} height={size - 14} preserveAspectRatio='xMidYMid meet' />
  </g>;
}
...
export default function PlayoffPoster({ tournament, matches, cup: selectedCup }) {
  ...
  if (selectedCup) {
    ...
    return <svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 1080 1350' role='img' aria-label={`Playoffs copa ${selectedCup} de ${tournament.name}`} style={{ width: '100%', display: 'block', fontFamily: 'Arial, sans-serif' }}>
      ...
      <TournamentLogos y={35} left={35} right={915} size={130} />
  ...
  return <svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 1080 2220' role='img' aria-label={`Cuadro de playoffs de ${tournament.name}`} style={{ width: '100%', display: 'block', fontFamily: 'Arial, sans-serif' }}>
    ...
    <TournamentLogos y={35} left={790} right={930} size={115} />
